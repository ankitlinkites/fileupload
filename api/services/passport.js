                ...
                if (user) {
                    ...
                    User.create(data).done(function (err, user) {
                      ...
                      if(err){
                        ...
                        UserProfile.create({userid:user.id}, function(error, p){
                          if(error){
                            console.log(error);
                            throw error;
                        ...
                        return done(null, user);
                          ...
                          if(profile.emails[0].value || !profile.emails[0].value == 'undefined'){
                              ...
                              if(err)
                                  ...
                                  UserProfile.create({userid:usr.id}, function(error, p){
                                    if(error){
                                      console.log(error);
                                      throw error;
                                      res.send({message : "error"});
    ...
    passport.use(new FacebookStrategy({
      ...
      User.create(user, function (err, new_usr) {
          if (err){ 
            ...
            UserProfile.create({userid:new_usr.id}, function(error, p){
              if(error){
                console.log(error);
                throw error;
                res.send({message : "error"});
